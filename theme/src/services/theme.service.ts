...
export class ThemeService {
  ...
  private getCssVar(name: string, value: string = ''): CssVar | undefined {
    ...
    if (value) {
      name = CSS_VARIABLES_PREFIX + this.utilsService.camelCaseToDashCase(name);
  ...
  public getThemeColor(theme: Theme): string {
    ...
    const primaryPalette: ColorPalette =
      theme.colorPalettes.find((cp) => cp.name === 'primary') ||
      ({} as ColorPalette);
    const primaryNormal: PaletteColor = primaryPalette.colors.find(
      (c: PaletteColor) => c.name === 'Normal'
    );
  ...
  public setTheme(themeName: string = '', forced: boolean = true): void {
    if (!forced) {
      if (typeof window !== 'undefined') {
        themeName =
          window.localStorage.getItem('selectedThemeName') || themeName;
      }
